...
def test_case():
    ...
    try:
        if (
            get_entropy_of_attribute(df, "outlook") >= 0.691
            and get_entropy_of_attribute(df, "outlook") <= 0.695
        ):
    ...
    try:
        if (
            get_entropy_of_attribute(df, "temp") >= 0.908
            and get_entropy_of_attribute(df, "temp") <= 0.914
        ):
    ...
    try:
        ...
        dictionary = ans[0]
        flag = (
            (dictionary["outlook"] >= 0.244 and dictionary["outlook"] <= 0.248)
            and (dictionary["temp"] >= 0.0292 and dictionary["temp"] <= 0.0296)
            and (dictionary["humidity"] >= 0.150 and dictionary["humidity"] <= 0.154)
            and (dictionary["windy"] >= 0.046 and dictionary["windy"] <= 0.05)
            and (ans[1] == "outlook")
        )
        if flag:
